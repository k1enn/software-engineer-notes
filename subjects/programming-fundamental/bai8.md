...
`StringBuilder` là một lớp trong C# thuộc namespace `System.Text`, được sử dụng để làm việc với chuỗi một cách hiệu quả, đặc biệt khi chuỗi cần được thay đổi nhiều lần (thêm, xóa, chèn, thay thế). Điều này giúp giảm việc tạo ra các chuỗi mới trong bộ nhớ, vốn xảy ra khi làm việc với kiểu `string` (vì `string` là **immutable** - không thể thay đổi).
