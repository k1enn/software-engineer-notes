...
namespace StackDoan
{
    internal class IntArray
    {
        ...
        #region Sắp xếp
        public void QuickSortDQ(int left, int right)
        {
            ...
            if(left < j) QuickSortDQ(left, j);
            if(i < right) QuickSortDQ(i, right);
        }
        public void QuickSort(int left, int right)
        {
            StackDoan listStack = new StackDoan();
            listStack.Push(new Doan { Left = left, Right = right });
            Doan doan = new Doan();
            while (!listStack.IsEmpty)
            {
                listStack.Pop(out doan);
                int i = doan.Left, j = doan.Right, x = arr[(doan.Left + doan.Right)/2];
                while(i<j)
                {
                    while (arr[i] < x) i++;
                    while (arr[j] > x) j--;
                ...
                if (doan.Left < j) listStack.Push(new Doan { Left = doan.Left, Right = j });
                if (i < doan.Right) listStack.Push(new Doan { Left = i, Right = doan.Right });
            }
        }
        ...
        public void NormalQuickSort(int[] a, int low, int high)
        {
            if (low <= high)
            {
                int pi = Partition(a, low, high);
                NormalQuickSort(a, low, pi - 1);
                NormalQuickSort(a, pi + 1, high);
            }
        }
